...
def get_country_data():
    ...
    html_data = get_html_data(url)
    bs = bs4.BeautifulSoup(html_data.text,'html.parser')
    subinfo_div = bs.find("div" , class_= "content-inner").findAll("div", id="maincounter-wrap")
    all_details = ""

    for blocks in subinfo_div:
        p = blocks.find("h1", class_=None)
        if p is not None:
            text = p.get_text()
        ...
        q= blocks.find("span", class_=None)
        if q is not None :
           count = q.get_text()
...
mainlabel = tk.Label(root, text=get_covid_data(),
font= f )
mainlabel.pack()
